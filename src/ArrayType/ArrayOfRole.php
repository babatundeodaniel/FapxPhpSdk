...
class ArrayOfRole extends AbstractStructArrayBase
{
    ...
    public static function validateRoleForArrayConstraintsFromSetRole(array $values = array())
    {
        $message = '';
        $invalidValues = [];
        foreach ($values as $arrayOfRoleRoleItem) {
            // validation for constraint: itemType
            if (!$arrayOfRoleRoleItem instanceof \StructType\Role) {
                $invalidValues[] = is_object($arrayOfRoleRoleItem) ? get_class($arrayOfRoleRoleItem) : sprintf('%s(%s)', gettype($arrayOfRoleRoleItem), var_export($arrayOfRoleRoleItem, true));
            }
        }
        if (!empty($invalidValues)) {
            $message = sprintf('The Role property can only contain items of type \StructType\Role, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
        return $message;
    }
    ...
    public function addToRole(\StructType\Role $item)
    {
        ...
        if (!$item instanceof \StructType\Role) {
            throw new \InvalidArgumentException(sprintf('The Role property can only contain items of type \StructType\Role, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->Role[] = $item;
        return $this;
    }
